...
7. What is the guarantee that the Insiders program is better than the rest of the base?
    - There's no guarantee at first, one could run an AB test to make shure that the insiders cluster performs better than the other clusters. Then, separate an A amount of clients from the insiders cluster and a B amount of clients from the other clusters for a specific amount of time and perform an AB testing.
